...
const PricingPage = () => {
  ...
  const plans = [
    {
      ...
      buttonStyle: "premium-chip-button text-white",
      bgColor: "bg-[#0A2463]",
      sections: [
        {
          title: "INCLUDES",
    ...
    },
    {
      ...
      buttonStyle:
        "border border-white/18 bg-[linear-gradient(135deg,#7FB22F_0%,#98C13F_100%)] text-white shadow-[0_18px_34px_rgba(152,193,63,0.22),inset_0_1px_0_rgba(255,255,255,0.22)] hover:-translate-y-0.5 hover:shadow-[0_24px_44px_rgba(152,193,63,0.28)]",
  ...
  return (
    <section data-header-tone="dark" className="mt-10 bg-[#020817] py-20 font-sans text-white">
      {/* Header Section */}
      <div className="site-shell mb-16 text-center">
        <h1 className="text-[36px] text-center md:text-[56px] font-semibold mt-20 mb-10">
          Choose the plan that fit your needs
        </h1>
        <p className="mx-auto max-w-4xl text-[16px] font-regular leading-relaxed text-white">
      ...
      {/* Pricing Cards Grid */}
      <div className="site-shell grid grid-cols-1 gap-6 lg:grid-cols-4">
        {plans.map((plan, index) => (
          <div
            key={index}
            // className={`${plan.bgColor} rounded-2xl p-6 lg:p-8 flex flex-col h-full border border-white/10`}
            className={`hover-lift premium-card relative w-full rounded-t-[26px] border-[1px] border-[#1763FA]/20 p-8 transition-all duration-300 lg:border-b-0
              ${
                plan.isFeatured
